...
public abstract class Parser {
	...
	/**
	 * Trie : collection of information to get referrals later.
	 *        trieID : xmi:id;
	 *        trieOpe : Behavior;
	 *        trieAssoc : uml:Association.
	 *        trieMetAbs : abstract methods 
	 *        trieAssocPacotes: package associations (dependecy)       
	 */
	...
	public static void runParser(String inputFileName) throws IOException {
		...
		while (bf.ready()) {
			 ...
			 */				
			//R: ENUMERATION
			if(line.contains("uml:Enumeration")){
				value = Tool.manipulate(line, "name");
				Enumeration enumeration = new Enumeration(value);
				model.addEnumeration(enumeration);
				enumeration.parser(bf,line);
			}			
