...
public class AttributeAndroid implements GeneratorStrategy {
	...
	public void codeGenerator(BufferedWriter out, int tab) throws IOException {	
		isUpperCase(this.attribute.getName());
		...
		if (attribute.getDefaultValue() != null || attribute.isStatic()) {
			...
			if (attribute.isPrimitiveType()) {
				...
				attribute.setName(attribute.getName().toUpperCase());
			}
		}
	...
	public boolean testSomeUpperCase(String str){
		for(int i=1; i<str.length()-1; i++){
			...
			if(!(c >= 97 && c <= 122)&& !(cn >= 97 && cn <= 122)) {
				this.attribute.setName(this.attribute.getName().substring(0, i+1)+ this.attribute.getName().substring(i+1, this.attribute.getName().length()).toLowerCase());
				return true;
			}
		}
		//str.charAt(index)
	...
	public void isUpperCase(String str){
		if(testAllUpperCase(str)){
			this.attribute.setName(this.attribute.getName().toLowerCase());
		}else {
