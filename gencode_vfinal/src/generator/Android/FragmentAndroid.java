...
public class FragmentAndroid implements GeneratorStrategy {
	...
	public void genCodeMessage(BufferedWriter out, int tab) throws IOException {
		if (fragment.isSend()) {
			...
			if(fragment.getCovered().isRepeated()){
				out.write("\n\t\t /** Sender: " + fragment.getCovered().getName()+ " */");
				

			}else{
				out.write("\n\t\t /** Sender: " + fragment.getCovered().getRepresents().getName()+ " */");
			}
	...
	private void genCodeAlt(BufferedWriter out, int tab) throws IOException {
		...
		if (fragment.getOperands().size() > 2) {
			...
			generatorOperand.genCodeOpt(out, tab);
			this.fragment.getOperands().get(1).done = true;
			}
	...
	//R: no repetition the code into combinated fragments (alt, loop)
	public boolean lastOperand(int i){
		lastOperand = false;
		for(Operand op : fragment.getOperands()){
			if((this.fragment.getOperands().get(i).equals(op))&&(op.done)){
				lastOperand = true;
			}
		}
		return lastOperand;
