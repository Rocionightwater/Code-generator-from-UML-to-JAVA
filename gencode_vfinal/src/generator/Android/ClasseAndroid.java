...
public class ClasseAndroid implements GeneratorStrategy{
	...
	public void codeGenerator(BufferedWriter out, int tab) throws IOException {
		...
		Logger.getGlobal().log(Level.INFO, "Writing class: " + getName());
		...
		try {
			...
			Parser.getModel().setDirectoryOut(cls.getAbsolutePath());
			// Pacote
...
//		
		...
		if (!(classe.isAbstract())) {
			...
			for (Attribute atr : classe.getAttributes()) {
				//R: if the attribute is ID
				if(atr.getIsID()) notInConstructor = true;
				if(atr.isFinal()) notInConstructor = true;
			}
			for (Attribute atr : classe.getAttributes()) {
				generatorAttribute = new AttributeAndroid(atr);
				if((!atr.getIsID())&&(!atr.isFinal())){	
					generatorAttribute.generatorConstructorSignature(out);
				}
			if(notInConstructor){	
				
					if ((!atr.getIsID()) && (i < classe.getAttributes().size() - 2)){
						out.write(",");
						i++;
					}
			}else{	
			...
			if(classe.getAttributes().size()>=2){
			//R: empty constructor
			out.write("\n\n" + tabInd + "\t/** Empty Constructor */");
			out.write("\n" + tabInd + "\tpublic " + classe.getName() + "(){");
			out.write("\n" + tabInd + "\t}\n");
		...
		if ((classe.needGetSet)) {
			...
			for (Attribute attr : classe.getAttributes()) {
				//R: attribute is only readable 
				if(!attr.getIsReadOnly()){
